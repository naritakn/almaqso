...
class QSOanalysis():
    ...
    def importasdm(self,dryrun=False):
        asdmfile = glob.glob(f'{os.getcwd()}/{os.path.basename(self.projID)}/*/*/*/raw/*')[0]
    ...
    def gen_calib_script(self,dryrun=False):
        ...
        if not dryrun:
            if os.path.exists('./log/'+self.visname + '.scriptForCalibration.py'):
                os.system('cp ./log/'+self.visname + '.scriptForCalibration.py ./')

            else:
                csg.generateReducScript(**kw_generateReducScript, useCalibratorService=False)
                # au.generateReducScript(**kw_generateReducScript)
                os.system('mkdir -p ./log')
                os.system('cp '+self.visname + '.scriptForCalibration.py ./log/')
    ...
    def avgspws(self,inputvis,dryrun=False):
        ...
        for spwID in spwIDs:
            data_new[:,0,:] = data_new[:,0,:] + data[:,0,ID==spwID]/sigma[:,ID==spwID]**2
    ...
    def uvfit_splitQSO_avg(self,spw,field,dryrun=False):
        ...
        if not dryrun:
            ...
            from casatasks import split,listobs,mstransform,delmod
            os.system('mkdir -p ./calibrated')
    ...
    def uvfit_uvmultifit(self,intent=None,write="",column='data',spwid='0',mfsfit=True,dryrun=False):
        ...
        if not dryrun:
            ...
            f.write('\n')
            f.write('kw_uvfit = {'+'\n')
            f.write('   "vis":"'+'calibrated/'+vis+'.'+self.field+'.spw_'+self.spw+'",'+'\n')
            f.write('   "spw":"0",'+'\n')
    ...
    def uvfit_gaincal(self,intent='phase',solint='int',solnorm=False,gaintype='G',calmode='p',gaintable='',dryrun=False):
        ...
        kw_gaincal = {
            ...
            'caltable':'./caltables/'+vis+'.'+self.field+'.spw_'+self.spw+'.'+intent,
            'field':'0',
            'solint':solint,
    ...
    def uvfit_man(self,datacolumn='data',intent=None,write_residuals=False,savemodel=False,dryrun=False,meansub=False,oldmode=True):
        ...
        if not dryrun:
            if not oldmode:
                ...
                tb = table()
                tb.open('calibrated/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw)
                if datacolumn == 'data':
                    data = tb.getcol('DATA').copy()
                elif datacolumn == 'corrected':
                    data = tb.getcol('CORRECTED_DATA').copy()
                flag = tb.getcol('FLAG').copy()
                weight = tb.getcol('WEIGHT').copy().reshape([2,1,flag.shape[2]])
                tb.close()
                ...
                res = least_squares(res_func,[1.0],args=(np.zeros_like(data),data))
                print('### fit ###')
                print(res.x[0])
                ...
                tb.open('calibrated/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw,nomodify=False)
                if savemodel:
                ...
                if intent == None:
                    infile = './specdata/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw+'.dat'
                else:
                    infile = './specdata/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw+'.'+intent+'.dat'
    ...
    def uvfit_run(self,dryrun=False,plot=True):
        for _field in self.fields:
            for _spw in self.spws:
                ...
                self.uvfit_applycal(gaintable=[gaintable_p,gaintable_ap],dryrun=dryrun,removeflag=False)
                ...
                if self.spacesave:
                    os.system('rm -rf calibrated/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw)
                    os.system('rm -rf calibrated/'+self.visname+'.split.split.'+self.field+'.spw_'+self.spw+'.listobs')
    ...
    def cont_imaging(self,clean=False,dryrun=False):
        ...
        if not dryrun:
            ...
            for field in self.fields:
                ...
                visForimsg = glob.glob('./calibrated/'+self.visname+'.split.split.'+field+'.spw_*.avg')
                ...
                if field[0] == 'J':
                    for spw in self.spws:
                        for selfcal in ['noselfcal','selfcal']:
                            try:
                                specfile = './specdata/'+self.visname+'.split.split.'+field+'.spw_'+spw+'.'+selfcal+'.dat'
                                ...
                                plt.ylim(ymin,ymax)
                                plt.savefig('./specplot/'+selfcal+'/'+self.asdmname + '.' + field + '.spw' + spw + '.pdf')
                                plt.savefig('./specplot/'+selfcal+'/'+self.asdmname + '.' + field + '.spw' + spw + '.png')
                                plt.close()
